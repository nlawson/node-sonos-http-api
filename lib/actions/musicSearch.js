...
const soundcloudDef = require('../music_services/soundcloudDef');
...
const musicServices = ['apple','spotify','soundcloud','deezer','elite','library'];
const serviceNames = {apple:'Apple Music',spotify:'Spotify',soundcloud:'SoundCloud',deezer:'Deezer',elite:'Deezer',library:'Library'};
...
function doSearch(service, type, term)
{
  ...
  } else if (type == 'station' && serviceDef.search['station'] == '') {
  	return []; // i.e. soundcloud doesn't have "stations" in sonos (yet)
  } else {
  ...
  return getAccountId(player, service)
    ...
    .then((resList) => {
      if (serviceDef.empty(type, resList)) {
		if(serviceDef.search[type] == '')
		  return Promise.reject('This service does not yet support ' + type + 's for Sonos.');
